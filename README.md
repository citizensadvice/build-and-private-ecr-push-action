...
| `multiarch_build`    | Allow for multi-arch builds. When `'disabled'` builds for the platform it is running in (usually `linux/amd64`), when `'enabled'` also builds `linux/arm64` images  | No       | `'enabled'`                                     |
